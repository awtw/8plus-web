      ...
      <header className="surface-card p-6 sm:p-8">
        <div className="flex flex-wrap items-center gap-3">
          <span className="eyebrow">
            ...
            {caseStudy ? t("projects.caseStudy") : "Lab"}
          </span>
          {isFallback && (
        ...
        {caseStudy && project.client && (
          <p className="mt-3 text-sm text-[color:var(--muted)]">
            <span className="font-medium text-[color:var(--fg)]">{t("projects.client")}：</span>
            {project.client}
          </p>
        )}
        ...
        {caseStudy && project.resultMetrics && project.resultMetrics.length > 0 && (
          <div className="mt-6 grid gap-3 sm:grid-cols-3">
            {project.resultMetrics.map((metric) => (
              <div key={metric.label} className="rounded-[var(--radius-md)] border border-border-soft bg-[color:var(--surface)] p-4">
                <p className="text-xs uppercase tracking-[0.16em] text-[color:var(--muted)]">{metric.label}</p>
                <p className="mt-1 text-xl font-semibold text-[color:var(--fg)]">{metric.value}</p>
              </div>
            ))}
          </div>
        )}
        ...
        <section className="mt-8 grid gap-4 lg:grid-cols-2">
          {project.challenge && (
            <div className="surface-card p-6 sm:p-7">
              <h2 className="text-xl font-semibold tracking-[-0.03em]">{t("projects.challenge")}</h2>
              <p className="mt-4 text-sm leading-7 text-[color:var(--fg-2)]">{project.challenge}</p>
            </div>
          )}
          {project.solution && (
            <div className="surface-card p-6 sm:p-7">
              <h2 className="text-xl font-semibold tracking-[-0.03em]">{t("projects.solution")}</h2>
              <p className="mt-4 text-sm leading-7 text-[color:var(--fg-2)]">{project.solution}</p>
            </div>
          )}
        </section>
      )}
